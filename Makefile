...
# ---------------------------------------------------------
clean-caches:
	@PYTHONDONTWRITEBYTECODE=1 python -B $(CACHE_CLEANER) || true
...
test:
	...
	@PYTHONDONTWRITEBYTECODE=1 NUMBA_DISABLE_JIT=1 PYTHONPATH=src python -B -m pytest -q
...
research:
	...
	@PYTHONDONTWRITEBYTECODE=1 NUMBA_DISABLE_JIT=1 PYTHONPATH=src python -B -m pytest -q -m slow -vv
...
perf:
	...
	@FISHBRO_PERF_BARS=20000 FISHBRO_PERF_PARAMS=1000 PYTHONPATH=src PYTHONDONTWRITEBYTECODE=1 python -B scripts/perf_grid.py

perf-mid:
	@echo "==> Running perf harness mid-tier (20000×10000, hot_runs=3, timeout=1200s)"
	@FISHBRO_PERF_BARS=20000 FISHBRO_PERF_PARAMS=10000 FISHBRO_PERF_HOTRUNS=3 FISHBRO_PERF_TIMEOUT_S=1200 PYTHONPATH=src PYTHONDONTWRITEBYTECODE=1 python -B scripts/perf_grid.py
...
perf-heavy:
	...
	@FISHBRO_PERF_BARS=200000 FISHBRO_PERF_PARAMS=10000 FISHBRO_PERF_HOTRUNS=1 FISHBRO_PERF_TIMEOUT_S=3600 PYTHONPATH=src PYTHONDONTWRITEBYTECODE=1 python -B scripts/perf_grid.py
...
release-txt:
	...
	@PYTHONDONTWRITEBYTECODE=1 python -B $(RELEASE_TOOL) txt

release-zip:
	@echo "==> Generating release ZIP (excludes .git)"
	@PYTHONDONTWRITEBYTECODE=1 python -B $(RELEASE_TOOL) zip
...
gui:
	...
	@python -c "import nicegui" 2>/dev/null || { echo "ERROR: nicegui not installed (pip install nicegui)"; exit 1; }
	@python -c "import streamlit" 2>/dev/null || { echo "ERROR: streamlit not installed (pip install streamlit)"; exit 1; }
	@bash -c 'trap "kill %1 %2 2>/dev/null || true; exit" EXIT INT TERM; \
		PYTHONPATH=src uvicorn FishBroWFS_V2.control.api:app --port 8000 & \
		PYTHONPATH=src python -m FishBroWFS_V2.control.app_nicegui & \
		PYTHONPATH=src streamlit run $(VIEWER_APP) --server.port 8502; \
		kill %1 %2 2>/dev/null || true'

demo:
	@echo "==> Creating demo job for Viewer validation"
	@PYTHONPATH=src python -m FishBroWFS_V2.control.seed_demo_run
