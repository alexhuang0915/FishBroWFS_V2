...
- **不可刪**：`control.supervisor.submit()`、`SupervisorDB.submit_job()`、`jobs_v2.db schema`、主線 handlers（BUILD_DATA / RUN_RESEARCH_WFS / BUILD_PORTFOLIO_V2）
...
- `src/control/shared_build.py` + `src/control/shared_cli.py`  
  BUILD_DATA 透過 CLI subprocess 走這條鏈，刪了會讓 BUILD_DATA 失效。
