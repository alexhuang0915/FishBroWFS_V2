#!/usr/bin/env python3
"""
Run minimal jobs for S1, S2, S3 and emit config reachability report (with cache clearing).
"""
...
# Import cached loaders to clear their caches
...
def clear_all_config_caches():
    """Clear LRU caches of all config loaders."""
...
def run_strategy_with_report(strategy_id: str, outputs_root: Path):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Clear caches and reset records
...
def main():
    ...
    print("Starting config reachability report generation (with cache clearing)...")
    print(f"Reports will be saved to {output_base}")
    
    # S1 fails due to missing features; we'll skip for now
    strategies = ["S2", "S3"]
