"""
Test RUN_FREEZE_V2 job lifecycle.
...
def test_submit_run_freeze_v2_job():
    """Test submitting a RUN_FREEZE_V2 job."""
    # Clean up any existing test jobs
    db = SupervisorDB(get_default_db_path())
    with db._connect() as conn:
        conn.execute("DELETE FROM jobs WHERE job_type = 'RUN_FREEZE_V2'")
    ...
    job_id = submit("RUN_FREEZE_V2", payload)
    
    # Verify job was created
    job = get_job(job_id)
    assert job is not None
    assert job.job_id == job_id
    assert job.job_type == "RUN_FREEZE_V2"
    assert job.state == "QUEUED"
    
    # Verify payload
    spec_dict = json.loads(job.spec_json)
    assert spec_dict["job_type"] == "RUN_FREEZE_V2"
    assert spec_dict["params"] == payload
    
    print(f"✓ Job submitted: {job_id}")


def test_run_freeze_v2_job_validation():
    """Test RUN_FREEZE_V2 parameter validation."""
    # Test missing required field
    with pytest.raises(ValueError, match="season"):
        submit("RUN_FREEZE_V2", {
            "force": False,
            "engine_version": "v2.1.0"
        })
...
def test_run_freeze_v2_handler_registered():
    """Test that RUN_FREEZE_V2 handler is registered."""
    from src.control.supervisor.job_handler import get_handler
    
    handler = get_handler("RUN_FREEZE_V2")
    assert handler is not None
    assert hasattr(handler, "validate_params")
    assert hasattr(handler, "execute")
    
    print("✓ RUN_FREEZE_V2 handler is registered")


@patch('src.control.supervisor.handlers.run_freeze.RunFreezeHandler._execute_freeze')
def test_run_freeze_v2_job_execution(mock_execute):
    """Test RUN_FREEZE_V2 job execution with mock."""
    ...
    mock_execute.return_value = {
        ...
        "result": {"output_files": ["freeze_manifest.json"]}
    }
...
def test_run_freeze_v2_manifest_generation():
    """Test manifest generation for RUN_FREEZE_V2 jobs."""
    from src.control.supervisor.handlers.run_freeze import RunFreezeHandler
...
if __name__ == "__main__":
    # Run tests
    test_submit_run_freeze_v2_job()
    test_run_freeze_v2_job_validation()
    test_run_freeze_v2_handler_registered()
    test_run_freeze_v2_manifest_generation()
