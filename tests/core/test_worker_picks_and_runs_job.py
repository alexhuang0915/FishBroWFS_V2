...
class TestWorkerPicksAndRunsJob(unittest.TestCase):
    def setUp(self) -> None:
        self._tmp = tempfile.TemporaryDirectory(prefix="fishbro_test_worker_")
    ...
    def test_worker_max_jobs_exits_and_job_done(self) -> None:
        ...
        # Run worker loop until it drains 1 job; must exit on its own.
        cmd = [
            sys.executable,
            "-m",
        ...
        self.assertIn(row.state, ("SUCCEEDED", "FAILED", "ABORTED", "REJECTED", "ORPHANED"))

        evidence_dir = self.outputs_root / "artifacts" / "jobs" / job_id
        self.assertTrue((evidence_dir / "spec.json").exists())
        self.assertTrue((evidence_dir / "state.json").exists())
        self.assertTrue((evidence_dir / "stdout.log").exists())
        self.assertTrue((evidence_dir / "stderr.log").exists())
